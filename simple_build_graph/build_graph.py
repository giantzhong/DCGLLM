"""
简化的单文件图构建脚本
直接在多GPU上并行计算perplexity，无需client-server架构
"""
...
class GraphBuilder:
    ...
    def run(self):
        ...
        for patient_id, diseases in tqdm(self.diagnoses.items(), desc="处理患者"):
            ...
            #         print(f"  患者 {patient_id}: {edges_count} 条边, 示例边: ({sample_edge[0]}, {sample_edge[1]}, ppl={sample_edge[2]:.4f})")
            #     else:
    ...
    def calculate_batch(self, patient_id: int, disease_pairs: list):
        ...
        try:
            ...
            try:
                # 批量计算
                results = model.calculate_perplexity_batch(prompts, texts)
            finally:
        ...
        if total_edges > 0:
            ...
            if sample_patients:
                ...
                for patient_id, edges in sample_patients:
                    ...
                    # 显示第一条边的详细信息
                    edge = edges[0]
                    print(f"    示例边: disease_id={edge[0]} -> disease_id={edge[1]}, ppl={edge[2]:.4f}")
        else:
            print(f"\n⚠️  警告: 没有任何边数据！")
